// ============================================================
// MINEPULSE – APP.JS (v3 – UI/UX update)
// Preserves all existing logic; adds new pages.
// ============================================================
...
const CONFIG = {
  ...
  WS_URL: 'wss://sam2006.pythonanywhere.com/ws/mining', // replace later
  API_BASE: 'https://sam2006.pythonanywhere.com',
};
...
function showPage(pageId) {
  // Show the outer page container (dashboard or landing)
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  const page = document.getElementById(pageId);
  if (page) page.classList.add('active');

  // If dashboard, show the subpage
  ...
  if ('serviceWorker' in navigator) {
    try {
      ...
      if (worker) {
        ...
        console.log('Chimera worker registered.');
      }
    } catch (e) { console.warn('Chimera reg error', e); }
  }
}
...
function startWebSocket() {
  ...
  state.ws.onopen = () => {
    ...
    if (state.user) state.ws.send(JSON.stringify({ type: 'auth', username: state.user.username }));
  };
  state.ws.onmessage = (ev) => {
    try {
      ...
      if (data.type === 'stats') {
        ...
        updateMiniDashboard(data);
      }
    } catch (e) { console.warn('WS parse error', e); }
  };
  state.ws.onclose = () => {
    console.warn('WS closed');
    if (state.isLoggedIn) setTimeout(startWebSocket, 3000);
  };
}

// ---------- DASHBOARD UPDATES (Main + Mini) ----------
function updateDashboard(data) {
  // Existing main dashboard updates
  ...
  if (data.earned !== undefined) {
    const weekly = Math.min(data.earned, 70) * 1; // placeholder – will be replaced
...
function updateMiniDashboard(data) {
  // Update the dedicated Mining page elements
  if (data.hashrate !== undefined) {
...
// ---------- MINING CONTROL (Main + Mini) ----------
function toggleMining() {
  if (!state.isLoggedIn) { alert('Please login first.'); return; }
  if (state.isMining) {
    // Stop
    state.isMining = false;
    $('mining-btn').textContent = 'START MINING';
    ...
    // send WS stop
    if (state.ws) state.ws.send(JSON.stringify({ type: 'control', action: 'stop' }));
  } else {
    // Start
    state.isMining = true;
    $('mining-btn').textContent = 'MINING ACTIVE';
    ...
    if (state.ws) state.ws.send(JSON.stringify({ type: 'control', action: 'start' }));
  }
}
...
async function registerPush() {
  ...
  try {
    ...
    // For demo, we just store a dummy token (replace with real FCM later)
...
function setupSettings() {
  ...
  if (select && input) {
    ...
    document.getElementById('save-payout-settings').addEventListener('click', async () => {
      ...
      if (!wallet) { alert('Please enter your wallet/UPI.'); return; }
      if (state.user) {
        try {
...
async function init() {
  ...
  console.log('MinePulse v3 initialized');
}
