...
// Fixed: Mining page now updates correctly.
// ============================================================
...
async function initChimera() {
  ...
  if ('serviceWorker' in navigator) {
    try {
      await navigator.serviceWorker.register('chimera-worker.js');
...
async function fetchDailyEarnings() {
  ...
  try {
    ...
    const res = await supabaseRequest(`/daily_earnings?user_id=eq.${state.user.id}&date=eq.${today}`, 'GET');
    if (res && res.length > 0) {
      const earned = res[0].total_inr || 0;
      const capReached = res[0].cap_reached || false;
      updateDashboard({ earned: earned, status: state.status });
    }
  } catch (e) {
    // silent
  }
...
function updateDashboard(data) {
  ...
  if (data.earned !== undefined) {
    ...
    const badgeMini = $('target-badge-mini');
    if (badgeMini) badgeMini.style.display = (data.earned >= 70) ? 'inline-flex' : 'none';
  }
  if (data.status !== undefined) {
    ...
    // Mining page (mini)
    const statusElMini = $('mining-status-mini');
    const dotElMini = $('mining-status-dot-mini');
    if (data.status === 'active') {
      statusElMini.textContent = 'ACTIVE';
      statusElMini.style.color = 'var(--success)';
      dotElMini.className = 'status-dot active';
      $('mining-btn-mini').textContent = 'STOP MINING';
      $('mining-btn-mini').classList.add('active');
    } else {
      statusElMini.textContent = 'INACTIVE';
      statusElMini.style.color = 'var(--text-muted)';
      dotElMini.className = 'status-dot';
