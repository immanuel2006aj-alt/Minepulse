...
// All alerts/confirms replaced with custom modal.
// Registration fixed for Supabase RLS + empty responses.
// Mining toggle now controls the Chimera worker.
// ============================================================
...
function showMessage(title, message, callback = null) {
  ...
  if (!modal) {
    // Fallback to native alert if modal doesn't exist (should never happen)
    alert(message);
  ...
  // Remove old listeners to avoid duplicate triggers
  ...
  // Click outside to close (acts like "OK")
...
async function handleRegister(e) {
  ...
  try {
    ...
    // Now fetch the user by username to get the UUID
...
// ---------- MINING CONTROL (UPDATED – sends commands to worker) ----------
